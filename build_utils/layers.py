...
class UpSamplingLayer(nn.Module):
    ...
    def forward(self, x):
        output= [] 
        for x_i in x:
            out = nn.functional.interpolate(x_i, scale_factor=2, mode='nearest')
            output.append(out)
...
class FeatureConcat(nn.Module):
    ...
    def forward(self, x, outputs):
        return torch.cat([outputs[i] for i in self.layers], dim=2) if self.multiple else outputs[self.layers[0]]
...
class WeightedFeatureFusion(nn.Module):  # weighted sum of 2 or more layers https://arxiv.org/abs/1911.09070
    ...
    def forward(self, x, outputs):
        ...
        # Fusion
        nx = x.shape[2]  # input channels(针对SNN优化)
        for i in range(self.n - 1):
            a = outputs[self.layers[i]] * w[i + 1] if self.weight else outputs[self.layers[i]]  # feature to add
            na = a.shape[2]  # feature channels
