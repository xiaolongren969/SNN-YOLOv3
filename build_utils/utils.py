...
from build_utils.utils import *   #  , google_utils
...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if nb:
            ...
            lbox = lbox.clone() + (1.0 - giou).mean()  # giou loss
            ...
            if model.nc > 1:  # cls loss (only if multiple classes)
                ...
                lcls = lcls.clone() + BCEcls(ps[:, 5:].clone(), t)  # BCE

            # Append targets to text file
            # with open('targets.txt', 'a') as file:
            #     [file.write('%11.5g ' * 4 % tuple(x) + '\n') for x in torch.cat((txy[i], twh[i]), 1)]

        lobj = lobj.clone() + BCEobj(pi[..., 4].clone(), tobj)  # obj loss

    # 乘上每种损失的对应权重
    lbox = lbox.clone() * h['giou']
    lobj *= lobj.clone() * h['obj']
    lcls *= lcls.clone() * h['cls']
