...
def train_one_epoch(model, optimizer, data_loader, device, epoch,
    ...
    for i, (imgs, targets, paths, _, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with amp.autocast(enabled=scaler is not None) and torch.autograd.set_detect_anomaly(True):
            pred = model(imgs)      
        ...
        losses = losses* (1. / accumulate)  # scale loss
...
def evaluate(model, data_loader, coco=None, device=None):
    ...
    for imgs, targets, paths, shapes, img_index in metric_logger.log_every(data_loader, 100, header):
        imgs = imgs.to(device).float() / 255.0  # uint8 to float32, 0 - 255 to 0.0 - 1.0
        targets = targets.to(device)
        # print("origin imgs shape：", imgs.shape)
        # 当使用CPU时，跳过GPU相关指令
        ...
        for index, p in enumerate(pred):
            if p is None:
                ...
                if len(imgs.shape) > 4:
                    imgs = imgs.squeeze_(0)
                #     print("输入图片为五维，使用squeeze_函数降低一维")
