...
def rate_spikes(data, timesteps):
    # 在脉冲传递的过程中，第一个维度压缩为T*N，后三个维度为chw，通过下列代码提取
    # data数据中的chw维度大小。
    chw = data.size()[1:]
    # 对时间维度进行平均，从而得到每个神经元的平均激发率。
...
class IFFunction(torch.autograd.Function):
    ...
    def forward(ctx, input, timesteps=10, Vth=1.0, alpha=0.5):
        ...
        # 将timesteps和Vth保存到ctx类中，以便在反向传递中使用。
    ...
    def backward(ctx, grad_output):
        with torch.no_grad():
            # 获取之前用ctx类保存的第一个tensors，即输入数据input。
            ...
            Vth_grad = torch.sum(Vth_grad) * 0.001
