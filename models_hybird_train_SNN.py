...
def create_modules(threshold: list, modules_defs: list, img_size):
    """
    ...
    for i, mdef in enumerate(modules_defs):
        ...
        if mdef["type"] == "convolutional":
            ...
            if mdef["activation"] == "leaky":
                ...
                modules.add_module("activation", neuron.IFNode(v_threshold=threshold[neuron_idx], surrogate_function=surrogate.ATan()))
                neuron_idx += 1
...
class Darknet(nn.Module):
    ...
    def __init__(self, cfg, T :int,img_size=(416, 416), verbose=False):
        ...
        self.threshold = [i*0.1 for i in [53.892    ,  21.858    ,  16.481   ,   12.622    ,  10.489   ,   5.7549   ,   25.675  ,    5.1418    ,  8.0158  ,    7.3881   ,   9.6547]]
        # 这里传入的img_size只在导出ONNX模型时起作用      
        self.input_size = [img_size] * 2 if isinstance(img_size, int) else img_size
        # 解析网络对应的.cfg文件
        self.module_defs = parse_model_cfg(cfg)
        # 根据解析的网络结构一层一层去搭建
        self.module_list, self.routs = create_modules(self.threshold, self.module_defs, img_size)
        # 获取所有YOLOLayer层的索引
