...
from build_utils.neuron import LIFNeuron, IFNeuron
    ...
    for i, mdef in enumerate(modules_defs):
        ...
        if mdef["type"] == "convolutional":
            ...
            if isinstance(k, int):
                ...
                #                                        bias=not bn))
                modules.add_module("Conv2d", layer.Conv2d(in_channels=output_filters[-1],
                                                          out_channels=filters,
                                                          kernel_size=k,
                                                          stride=stride,
                                                          padding=k // 2 if mdef["pad"] else 0,
            ...
            if bn:
                ...
                modules.add_module("BatchNorm2d", layer.BatchNorm2d(filters))
            else:
            ...
            if mdef["activation"] == "leaky":
                ...
                modules.add_module("activation", neuron.IFNode(surrogate_function=surrogate.ATan()))
                # modules.add_module("actication", IFNeuron(snn_setting))
            else:
                pass
            ...
            maxpool = layer.MaxPool2d(kernel_size=k, stride=stride, padding=(k - 1) // 2)
            ...
            yolo_index = yolo_index + 1  # 记录是第几个yolo_layer [0, 1, 2]
            ...
            try:
                ...
                b.data[:, 4] = b.data[:, 4].clone() - 4.5  # obj
                b.data[:, 5:] = b.data[:, 5:].clone() + math.log(0.6 / (modules.nc - 0.99))  # cls (sigmoid(p) = 1/nc)
...
class YOLOLayer(nn.Module):
    ...
    def forward(self, p):
        ...
        if self.training:
            ...
            io[..., :2] = torch.sigmoid(io[..., :2].clone()) + self.grid  # xy 计算在feature map上的xy坐标
            io[..., 2:4] = torch.exp(io[..., 2:4].clone()) * self.anchor_wh  # wh yolo method 计算在feature map上的wh
            io[..., :4] = io[..., :4].clone() * self.stride  # 换算映射回原图尺度
            torch.sigmoid_(io[..., 4:])
...
class Darknet(nn.Module):
    ...
    """
    def __init__(self, cfg, T :int,img_size=(416, 416), verbose=False):
    ...
    def forward(self, x, verbose=False):
        x_ = copy.deepcopy(x)
        self.forward_once(x, verbose=verbose)
        print(len(self.out))
        self.out.insert(0, x_.unsqueeze_(0).repeat(self.T, 1, 1, 1, 1))
        print(len(self.out))
        del x_
        layer_num = 0
        Conv_idx=0
        Conv_mem = np.zeros((11))
        for module in self.module_list:
                if isinstance(module, nn.Sequential):
                    for sub_module in module:
                        if isinstance(sub_module, nn.BatchNorm2d):
                            # print(f"input:{self.out[layer_num].shape}")
                            # print(f"layer_num:{layer_num}")
                            # print(f"module:{list(sub_module.parameters())[0].shape}")
                            # print("------------------------")
                            # self.out[layer_num]是为了某一访问卷积层的输入
                            # module[0] 是让卷积层的输入量通过卷积层得到结果
                            # module[1] 是让卷积层得到的结果再经过BatchNorm2d的到IF前的值
                            if(module[1](module[0](self.out[layer_num]))).max()>Conv_mem[Conv_idx]:
                                Conv_mem[Conv_idx] = module[1]((module[0](self.out[layer_num]))).max()
                                Conv_idx += 1
                    layer_num += 1
                else:
                    layer_num += 1
    ...
    def forward_once(self, x, verbose=False):
        ...
        for i, module in enumerate(self.module_list):
            ...
            if name in ["WeightedFeatureFusion", "FeatureConcat"]:  # sum, concat
                ...
                pass
                # yolo_out.append(module(x.mean(dim=0)))

            else:  # run module directly, i.e. mtype = 'convolutional', 'upsample', 'maxpool', 'batchnorm2d' etc.
                x = module(x)

            #如果在训练阶段，保存需要使用到的output，测试阶段保存所有out
            if self.training:  
